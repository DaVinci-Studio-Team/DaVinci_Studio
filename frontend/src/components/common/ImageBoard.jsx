...
const ImageBoard = ({ loading, images = [], error }) => {
        ...
        {loading ? (
          <div className="flex gap-6 pb-4 overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-4 md:overflow-visible">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="flex-shrink-0 w-64 h-64 overflow-hidden rounded-lg md:w-full animate-pulse"
              >
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-3 text-center border border-red-500/20 rounded-xl bg-red-500/5">
            ...
            <button
              ...
              className="px-4 py-2 mt-2 text-sm text-white transition rounded-lg bg-red-500/20 hover:bg-red-500/30"
            >
              Try Again
            </button>
          </div>
